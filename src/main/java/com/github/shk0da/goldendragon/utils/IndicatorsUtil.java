...
public class IndicatorsUtil {
    ...
    public static double obv(double[] inClose, double[] inVolume) {
        ...
        RetCode retCode =
                talib.obv(
                        0, inClose.length - 1, inClose, inVolume, outBegIdx, outNBElement, outReal);
    ...
    public static List<TickerCandle> convertCandles(
        ...
        try {
            var currentTimestamp =
                    ofInstant(
                            dateTimeFormat.parse(candles.get(0).getDate()).toInstant(),
                            systemDefault());
            for (TickerCandle candle : candles) {
                open = 0.0 == open ? candle.getOpen() : open;
                var timestamp =
                        ofInstant(
                                dateTimeFormat.parse(candle.getDate()).toInstant(),
                                systemDefault());
