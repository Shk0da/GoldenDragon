...
public final class GoldenDragon {
    ...
    private static final String DEFAULT_MARKET = "MOEX";
    ...
    public static void main(final String[] args) {
        ...
        try {
            ...
            final String market = getMarket(args);
            final String accountId = getAccountId(args, mainConfig);
            out.println("Run: " + strategy + " " + market + " [" + accountId + "]");

            final String baseCurrency = getBaseCurrency(market);
            final TCSService tcsService =
                    new TCSService(mainConfig.withAccountId(accountId), baseCurrency);
    ...
    private static String getMarket(final String[] args) {
        return args.length > ACCOUNT_ARG_INDEX ? args[ACCOUNT_ARG_INDEX] : DEFAULT_MARKET;
    }
    ...
    private static String getBaseCurrency(String market) {
        // Simple currency mapping - can be extended if needed
        if ("MOEX".equalsIgnoreCase(market)) {
            return "RUB";
        }
        return "RUB";
    }
