...
public class IndicatorsUtil {
  ...
  public static Map<String, List<Indicator>> initializeIndicators(List<HistoricCandle> candles) {
      ...
      MABlack.add(
          new Indicator(
              ...
              LocalDateTime.ofInstant(
                  Instant.ofEpochSecond(candles.get(j).getTime().getSeconds()),
    ...
    for (int i = chunkSmaMacdSize; i < MACD.size(); i++) {
      ...
      MACD_SMA.add(
          new Indicator("MACD_SMA", value, MACD.get(j).getDateTime(), MACD.get(j).getValue()));
    }
    ...
    for (int i = chunkRsiSmaSize; i < RSI.size(); i++) {
      ...
      double value = sma(inClose);
      RSI_SMA.add(new Indicator("RSI_SMA", value, RSI.get(j).getDateTime(), RSI.get(j).getValue()));
    }
    ...
    for (int i = chunkObvSmaSize; i < OBV.size(); i++) {
      ...
      double value = sma(inClose);
      OBV_SMA.add(new Indicator("OBV_SMA", value, OBV.get(j).getDateTime(), OBV.get(j).getValue()));
    }
  ...
  public static double obv(double[] inClose, double[] inVolume) {
    ...
    RetCode retCode =
        talib.obv(0, inClose.length - 1, inClose, inVolume, outBegIdx, outNBElement, outReal);
    return (RetCode.Success == retCode && outNBElement.value > 0)
        ? outReal[outNBElement.value - 1]
        : 0.0;
  }
  ...
  public static double rsi(double[] inClose) {
    ...
    MInteger outNBElement = new MInteger();
    RetCode retCodeRSI =
        talib.rsi(0, inClose.length - 1, inClose, RSI_PERIOD, outBegIdx, outNBElement, outReal);
    return (RetCode.Success == retCodeRSI && outNBElement.value > 0)
        ? outReal[outNBElement.value - 1]
        : 0.0;
  }
  ...
  public static double adx(double[] inClose, double[] inLow, double[] inHigh) {
    ...
    RetCode retCodeADX =
        talib.adx(
            0, inClose.length - 1, inHigh, inLow, inClose, ADX_PERIOD, beginADX, lengthADX, outADX);
    return (RetCode.Success == retCodeADX && lengthADX.value > 0)
        ? outADX[lengthADX.value - 1]
        : 0.0;
  }
  ...
  public static double movingAverageBlack(double[] inClose) {
    ...
    RetCode retCodeBlackMA =
        talib.trima(
            ...
            lengthBlackMA,
            outBlackMA);
    return (RetCode.Success == retCodeBlackMA && lengthBlackMA.value > 0)
        ? outBlackMA[lengthBlackMA.value - 1]
        : 0.0;
  }
  ...
  public static double movingAverageWhite(double[] inClose) {
    ...
    RetCode retCodeWhiteMA =
        talib.trima(
            ...
            lengthWhiteMA,
            outWhiteMA);
    return (RetCode.Success == retCodeWhiteMA && lengthWhiteMA.value > 0)
        ? outWhiteMA[lengthWhiteMA.value - 1]
        : 0.0;
  }
  ...
  public static double sma(double[] inClose) {
    ...
    MInteger outNBElement = new MInteger();
    RetCode retCode =
        talib.sma(0, inClose.length - 1, inClose, SMA_PERIOD, outBegIdx, outNBElement, outReal);
    return (RetCode.Success == retCode && outNBElement.value > 0)
        ? outReal[outNBElement.value - 1]
        : 0.0;
  }
  ...
  private static double stochasticCross(
      double[] closePrices,
      double[] minPrices,
      double[] maxPrices,
      int fastKPeriod,
    ...
    return RetCode.Success == retCode
            && length.value > 0
            && outK[length.value - 1] > outD[length.value - 1]
            && outK[length.value - 2] <= outD[length.value - 2]
        ? 1.0
        : 0.0;
  }
  ...
  public static List<TickerCandle> convertCandles(
    ...
    try {
      var currentTimestamp =
          ofInstant(dateTimeFormat.parse(candles.get(0).getDate()).toInstant(), systemDefault());
      for (TickerCandle candle : candles) {
        open = 0.0 == open ? candle.getOpen() : open;
        var timestamp =
            ofInstant(dateTimeFormat.parse(candle.getDate()).toInstant(), systemDefault());
        if (timestamp.isEqual(currentTimestamp.plus(newTimeFrame, unit))) {
          newCandles.add(
              new TickerCandle(
                  candle.getSymbol(),
                  dateTimeFormat.format(
                      Date.from(currentTimestamp.atZone(systemDefault()).toInstant())),
                  open,
                  high,
                  low,
                  candle.getClose(),
