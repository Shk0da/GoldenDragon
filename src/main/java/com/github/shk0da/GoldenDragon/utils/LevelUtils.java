...
public class LevelUtils {
    ...
    public static class Level {
        ...
        void mergeFrom(Level other) {
            ...
            if (combinedVolume > 0) {
                this.price =
                        (this.price * this.totalVolume + other.price * other.totalVolume)
                                / combinedVolume;
            }
        ...
        public String toString() {
            return String.format(
                    "Level{%.5f} | Type: %s | Touches: %d | Volume: %.0f | Strength: %.0f",
     ...
     */
    public LevelUtils(
            double levelZonePercent,
            int minTouchesForLevel,
            double minStrengthPercentile,
            int consolidationWindow,
            double consolidationThreshold) {
        this(
                levelZonePercent,
                minTouchesForLevel,
                minStrengthPercentile,
                consolidationWindow,
    ...
    /**
     * Основной метод для поиска ключевых уровней. Комбинирует несколько методов определения уровней
     * по стратегии Герчика: 1. Локальные экстремумы (с адаптивным окном) 2. Зоны консолидации 3.
     * Уровни, от которых были сильные отбои (длинные тени)
     */
    ...
    /**
     * Поиск локальных экстремумов. Используем адаптивное окно на основе размера истории. Учитываем
     * объём — приоритет экстремумам с высоким объёмом.
     */
    private void findLocalExtremes(
            List<TickerCandle> candles, List<Level> levels, double avgVolume) {
    ...
    /**
     * Поиск зон консолидации. Если в окне из N свечей разброс цен закрытия мал — это горизонтальный
     * уровень. Улучшенная логика определения типа уровня (поддержка/сопротивление).
     */
    ...
    /**
     * Поиск отбоев по длинным теням. Длинная нижняя тень = отбой от поддержки, длинная верхняя =
     * отбой от сопротивления. Использует динамические пороги на основе среднего диапазона свечей.
     */
    private void findWickRejections(
            List<TickerCandle> candles, List<Level> levels, double avgVolume) {
        // Предварительный расчёт среднего диапазона для динамических порогов
        double avgRange =
                candles.stream().mapToDouble(c -> c.getHigh() - c.getLow()).average().orElse(1);
        ...
        for (TickerCandle candle : candles) {
            ...
            if (relativeLowerWick >= wickRatioThreshold && relativeBody <= bodyRatioThreshold) {
                ...
                addLevelWithMultiplier(
                        levels, candle.getLow(), true, candle.getVolume(), volumeMultiplier);
            }
            ...
            if (relativeUpperWick >= wickRatioThreshold && relativeBody <= bodyRatioThreshold) {
                double volumeMultiplier = highVolume ? 1.5 : 1.0;
                addLevelWithMultiplier(
                        levels, candle.getHigh(), false, candle.getVolume(), volumeMultiplier);
            }
        }
    }

    /** Добавляет уровень с множителем объёма (для усиления значимости). */
    private void addLevelWithMultiplier(
            List<Level> levels, double price, boolean isSupport, double volume, double multiplier) {
    ...
    private List<Level> mergeNearbyLevels(List<Level> levels) {
        ...
        for (int i = 1; i < levels.size(); i++) {
            ...
            if (last.isSupport() == current.isSupport()
                    && withinZone(last.getPrice(), current.getPrice())) {
    ...
    /**
     * Фильтрация уровней с комбинированным скорингом. Учитывает: количество касаний (40%), силу
     * уровня (40%), объём (20%).
     */
    private List<Level> filterSignificantLevels(List<Level> levels) {
        ...
        double maxStrength = levels.stream().mapToDouble(Level::getStrength).max().orElse(1);
        int maxTouches = levels.stream().mapToInt(Level::getTouches).max().orElse(1);
        double maxVolume = levels.stream().mapToDouble(Level::getTotalVolume).max().orElse(1);
