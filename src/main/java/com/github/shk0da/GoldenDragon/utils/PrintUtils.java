...
public final class PrintUtils {
  ...
  public static void printCurrentPositions(
    ...
    out.printf(
        "%-8s %-8s %-15s %-15s %-9s %-5s %-5s \n",
        "Ticker", "Type", "FIGI", "ISIN", "Qty", "Lots", "Yield");
    out.println("--------------------------------------------------------------------------");

    Map<TickerInfo.Key, PositionInfo> sortedPositions =
        currentPositions.entrySet().stream()
    ...
    for (Map.Entry<TickerInfo.Key, PositionInfo> entry : sortedPositions.entrySet()) {
      ...
      if (currency.equals(tickerInfo.getCurrency())) {
        totalPortfolioCost +=
            tcsService.getAvailablePrice(tickerInfo.getKey(), balance, false) * balance;
      }
    }
    out.println("==========================================================================\n");
  ...
  public static void printCalendarOfDividends(Map<String, List<DiviTicker>> dividends) {
    ...
    out.printf(
        "%-10s %-8s %-12s %-8s %-12s %-8s %-10s \n",
        "Close Date", "Ticker", "Description", "Price", "Dividend", "Percent", "Close Date");
    out.println("--------------------------------------------------------------------------");
  ...
  public static void printCurrentPositions(
    ...
    out.println(
        "================================= Current Positions ================================");
    out.printf(
        "%-8s %-8s %-15s %-15s %-9s %-5s %-12s %-5s \n",
        "Ticker", "Type", "FIGI", "ISIN", "Qty", "Lots", "Price", "  %");
    out.println(
        "------------------------------------------------------------------------------------");

    Map<TickerInfo.Key, PortfolioPosition> sortedPositions =
        currentPositions.entrySet().stream()
