...
/**
 * Динамически вычисляемые параметры: - maxSignalAge = на основе ATR и волатильности -
 * volumeConfirmationThreshold = на основе перцентиля объёмов - breakout/falseBreakout множители =
 * на основе volatility ratio - pattern strength = на основе тренда
 */
public class GerchikUtils {
    ...
    /** Минимальное количество касаний уровня для подтверждения. Оптимально: 3-4 */
    public final int levelConfirmationTouches;

    /** Процентная зона вокруг уровня (в % от цены). Оптимально: 0.008-0.012 (0.8%-1.2%) */
    public final double levelZonePercent;

    /** Количество свечей для подтверждения сигнала. Оптимально: 3-4 */
    public final int confirmationCandles;

    /** Минимальная сила паттерна для входа. Оптимально: 0.8-1.2 */
    ...
    public GerchikUtils(String name) {
        ...
        this.levelConfirmationTouches =
                Integer.parseInt(
                        (null
                                        != presetProperties.getProperty(
                                                "levelTrader.levelConfirmationTouches"))
                                ? presetProperties.getProperty(
                                        "levelTrader.levelConfirmationTouches")
                                : defaultProperties.getProperty(
                                        "levelTrader.levelConfirmationTouches", "3"));
        this.levelZonePercent =
                Double.parseDouble(
                        (null != presetProperties.getProperty("levelTrader.levelZonePercent"))
                                ? presetProperties.getProperty("levelTrader.levelZonePercent")
                                : defaultProperties.getProperty(
                                        "levelTrader.levelZonePercent", "0.01"));
        this.confirmationCandles =
                Integer.parseInt(
                        (null != presetProperties.getProperty("levelTrader.confirmationCandles"))
                                ...
                                : defaultProperties.getProperty(
                                        "levelTrader.confirmationCandles", "3"));
        this.minPatternStrength =
                Double.parseDouble(
                        (null != presetProperties.getProperty("levelTrader.minPatternStrength"))
                                ...
                                : defaultProperties.getProperty(
                                        "levelTrader.minPatternStrength", "1.0"));
    }

    // Полный конструктор
    public GerchikUtils(
            int levelConfirmationTouches,
    ...
    /**
     * Динамический расчёт maxSignalAge на основе волатильности. Чем выше волатильность, тем меньше
     * возраст сигнала.
     */
        ...
        boolean longSignal =
                signals.stream()
                        .filter(
                                s ->
                                        !s.isFalseBreakout()
                                                && s.getConfirmationStrength() > minPatternStrength)
                        .anyMatch(
                                s ->
                                        (s.getLevel().isSupport()
                                                        && s.getType() == SignalType.BOUNCE)
                                                || (!s.getLevel().isSupport()
                                                        && s.getType() == SignalType.BREAKOUT));
    ...
    private List<TradingSignal> processMarketData(
    ...
    private void markFalseBreakouts(
            List<TickerCandle> candles, List<TradingSignal> signals, AtrContext atrCtx) {
    ...
    private List<Level> identifyKeyLevels(List<TickerCandle> candles, List<Double> priceLevels) {
        ...
        for (int i = 0; i < candles.size(); i++) {
            ...
            Optional<Double> support =
                    priceLevels.stream().filter(p -> p <= close).max(Double::compareTo);
            Optional<Double> resistance =
                    priceLevels.stream().filter(p -> p > close).min(Double::compareTo);
    ...
    private void checkAndUpdateLevel(
            List<Level> levels, double price, boolean isSupport, double volume, int timestamp) {
    ...
    private boolean isBreakout(
            Level level, TickerCandle prev, TickerCandle curr, AtrContext atrCtx) {
    ...
    private double calculateConfirmationStrength(
            Level level, List<TickerCandle> candles, int idx, double currentAtr) {
    ...
    private List<TradingSignal> filterByTime(List<TradingSignal> signals, int maxSignalAge) {
        ...
        return signals.stream()
                ...
                .sorted(
                        Comparator.comparingDouble(TradingSignal::getConfirmationStrength)
                                .reversed())
                .collect(Collectors.toList());
    }
