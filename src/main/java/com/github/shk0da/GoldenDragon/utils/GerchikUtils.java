...
public class GerchikUtils {
      ...
      for (Level level : levels) {
        ...
        if (isBreakout(level, prev, curr, atrCtx)) {
          ...
          if (volOk && patOk) {
            ...
            signals.add(
                new TradingSignal(
                    SignalType.BREAKOUT, curr.getClose(), level, conf, false, i, curr.getVolume()));
          }
        }
        ...
        if (isBounce(level, prev, curr)) {
          ...
          if (patOk || volOk) {
            ...
            signals.add(
                new TradingSignal(
                    SignalType.BOUNCE, curr.getClose(), level, conf, false, i, curr.getVolume()));
          }
        }
      }
    }
    ...
    for (TradingSignal s : signals) {
      ...
      for (int i = start; i < end; i++) {
        ...
        if (s.getLevel().isSupport()) {
          returnedIntoZone =
              c.getClose() > baseLevel && (c.getClose() - baseLevel) <= returnThreshold;
        } else {
          returnedIntoZone =
              c.getClose() < baseLevel && (baseLevel - c.getClose()) <= returnThreshold;
        }
  ...
  private boolean isBounce(Level level, TickerCandle prev, TickerCandle curr) {
    ...
    if (level.isSupport()) {
      ...
      boolean reversal =
          bullish(curr) && curr.getClose() > prev.getClose() && curr.getLow() >= prev.getLow();
      ...
      boolean reversal =
          bearish(curr) && curr.getClose() < prev.getClose() && curr.getHigh() <= prev.getHigh();
      boolean closeNear = curr.getClose() >= p * (1 - levelZonePercent * 4);
      return touched && reversal && closeNear;
    }
  }
