...
/**
 * Service for downloading and converting historical data from Bybit.
 * Integrates with DataCollector to provide crypto market data.
 *
 * Encapsulates all Bybit-related functionality:
 * - Downloading raw tick/trade data
 * - Converting to candlestick format (5-min, 1-hour)
 * - Managing local data files
 */
public class BybitService {

    private static final String BASE_URL = "https://public.bybit.com/";
    private static final List<String> KNOWN_DATA_TYPES = Arrays.asList(
    ...
    /**
     * Download and convert historical data for specified crypto tokens.
     * Downloads and converts incrementally - converts each file immediately after download.
     * Processes multiple coins in parallel.
     *
     ...
     */
    public void downloadAndConvert(List<String> coins, LocalDate startDate, LocalDate endDate) throws IOException {
        ...
        log("Processing " + coins.size() + " coins in parallel with " + COIN_DOWNLOAD_THREADS + " threads");

        try {
            java.util.concurrent.ExecutorService executor = java.util.concurrent.Executors.newFixedThreadPool(COIN_DOWNLOAD_THREADS);
            java.util.concurrent.CountDownLatch latch = new java.util.concurrent.CountDownLatch(coins.size());
     ...
     */
    private void downloadAndConvertSingleCoin(String coin, LocalDate startDate, LocalDate endDate) throws Exception {
        ...
        BybitDataDownloader downloader = new BybitDataDownloader(
                coinDataDir,
                coin,
                startDate,
                endDate,
    ...
    /**
     * Download raw trading data from Bybit.
     */
    private void downloadRawData(List<String> coins, LocalDate startDate, LocalDate endDate) throws Exception {
        log("Downloading raw trading data from Bybit...");
        ...
        for (String coin : coins) {
            ...
            BybitDataDownloader downloader = new BybitDataDownloader(
                    ...
                    startDate,
                    endDate,
                    null  // No incremental converter, will convert all at once later
            );
            downloader.downloadAll();
        }

        log("Raw data download completed");
    }

    /**
     * Convert downloaded tick data to candlestick format.
     */
    private void convertToCandles(List<String> coins) throws Exception {
        log("Converting tick data to candlesticks...");

        for (String coin : coins) {
            try {
                String dataDir = Paths.get(this.dataDir, coin).toString();

                // Check if input directory exists and has files
                Path inputPath = Paths.get(dataDir);
                if (!Files.exists(inputPath)) {
                    log("No downloaded data found for " + coin + ", skipping conversion");
                    continue;
                }

                long csvCount = Files.walk(inputPath)
                        .filter(p -> p.toString().endsWith(".csv"))
                        .count();

                if (csvCount == 0) {
                    log("No CSV files found for " + coin + ", skipping conversion");
                    continue;
                }

                log("Converting " + coin + " for " + csvCount + " CSV files...");

                CandleConverter converter = new CandleConverter(dataDir, dataDir);
                converter.convertAll();

                log("Conversion completed for " + coin);
            } catch (Exception e) {
                logError("Failed to convert data for " + coin, e);
                throw e;
            }
        }

        log("Candlestick conversion completed");
    }

    /**
     * Check if data needs to be updated (last download was before today).
     *
     * @param coin Coin pair to check
     * @return true if update is needed
     */
    public boolean needsUpdate(String coin) {
        try {
            Path coinDir = Paths.get(dataDir, coin);
            if (!Files.exists(coinDir)) {
                return true;
            }

            Path candlesFile = coinDir.resolve("candlesHOUR.txt");
            if (!Files.exists(candlesFile)) {
                return true;
            }

            // Check if file was modified today
            long lastModified = Files.getLastModifiedTime(candlesFile).toMillis();
            long today = LocalDate.now().atStartOfDay().atZone(java.time.ZoneId.systemDefault()).toInstant().toEpochMilli();

            return lastModified < today;
        } catch (IOException e) {
            log("Error checking update status for " + coin + ": " + e.getMessage());
            return true;
        }
    }
    ...
    /**
     * Internal downloader for Bybit historical data.
     * Not exposed outside BybitService.
     */
    private static class BybitDataDownloader {
        ...
        BybitDataDownloader(String outputDir, String targetCoin,
        ...
        void downloadAll() throws Exception {
            ...
            System.out.println("Starting multi-threaded download with " + DOWNLOAD_THREADS + " threads...");
                ...
                for (String link : links) {
                    if (link.toLowerCase().endsWith(".csv.gz")) {
                        ...
                        if (fileDate != null && !fileDate.isBefore(startDate) && !fileDate.isAfter(endDate)) {
                            String extractedFileName = link.endsWith(".gz") ? link.substring(0, link.length() - 3) : link;
        ...
        private void downloadFilesMultiThreaded(List<FileToDownload> files) throws Exception {
            java.util.concurrent.ExecutorService executor = java.util.concurrent.Executors.newFixedThreadPool(DOWNLOAD_THREADS);
            ...
            for (FileToDownload file : files) {
                executor.submit(() -> {
                    try {
                        System.out.println("Downloading [" + (downloadedCount.get() + 1) + "/" + files.size() + "]: " + 
                                         file.fileName + " (date: " + file.date + ")");
                        
                        Path extractedPath = Paths.get(file.outputDir, 
                            file.fileName.endsWith(".gz") ? file.fileName.substring(0, file.fileName.length() - 3) : file.fileName);
    ...
    /**
     * Internal converter for transforming tick data to candlesticks.
     * Not exposed outside BybitService.
     */
    private static class CandleConverter {

        private static final DateTimeFormatter OUTPUT_DATE_FORMAT = DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm:ss");
        ...
        void convertAll() throws Exception {
            ...
            List<Path> csvFiles = Files.walk(inputDir)
                    .filter(p -> p.toString().endsWith(".csv"))
                    .toList();
            ...
            for (Path csvFile : csvFiles) {
                fileCount++;
                if (fileCount % 50 == 0) {
                    System.out.println("Reading file " + fileCount + "/" + csvFiles.size() + ": " + csvFile.getFileName());
                }
            ...
            System.out.println("Written " + candles5Min.size() + " 5-minute candles to " + candles5MinFile);
    ...
    /**
     * Incremental converter for converting single CSV files to candles.
     * Accumulates ticks and updates candle files incrementally.
     */
    private static class CandleIncrementalConverter {
        ...
        synchronized void convertSingleFile(Path csvFile) {
            try {
                ...
                if (!allTicks.isEmpty()) {
                    ...
                    System.out.println("Converted " + csvFile.getFileName() + " - Total ticks: " + allTicks.size() + 
                                     ", 5-min candles: " + candles5Min.size() + 
                                     ", 1-hour candles: " + candles1H.size());
                }
            } catch (Exception e) {
        ...
        private void writeCandlesFile(Path outputFile, List<Candle> candles, boolean writeHeader) throws IOException {
            try (BufferedWriter writer = Files.newBufferedWriter(outputFile)) {
                if (writeHeader) {
                    writer.write("Datetime,Open,High,Low,Close,Volume");
                    writer.newLine();
        ...
        private void writeTickerJson(Path outputFile, String tickerName) throws IOException {
            ...
            String name = tickerName != null && !tickerName.isEmpty() ? tickerName : dataDir.getFileName().toString();
