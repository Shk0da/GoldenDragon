...
public class MarketRegimeFilter {
    ...
    public static class FilterResult {
        ...
        public FilterResult(boolean canTrade, double confidence, double positionMultiplier, String reason) {
    ...
    public FilterResult evaluate(List<Candle> candles,
        ...
        if (adx.adx <= adxUnclearThreshold) {
            ...
            if (diStrong && atrGrowing) {
                confidence = Math.min(adxUnclearThreshold - 1, adxRangeThreshold * 0.5 + (adx.adx - adxRangeThreshold) * 5);
                posMult = 0.5;
                regime = "TREND_EARLY";
            } else {
        ...
        boolean atrExpanding = currentAtr > smoothedAtr && isAtrGrowing(candles, atrPeriod, atrBars);
        ...
        return new FilterResult(true, confidence, posMult,
    ...
    private double calculateSmoothedAtr(List<Candle> candles, int period) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (int j = end - period; j < end; j++) {
                ...
                atr += Math.max(Math.max(c.high - c.low, Math.abs(c.high - p.close)), Math.abs(c.low - p.close));
            }
            sum += atr / period;
            validCount++;
        }
