 ...
 *
 * <p>Является обёрткой над {@link UnifiedStrategy}, добавляя два дополнительных слоя принятия
 * решений: определение рыночного режима на основе ADX и фильтрацию/масштабирование сделок с помощью
 * ML-модели классификации.
 *
 * <h2>1. Определение рыночного режима</h2>
 *
 * <p>На часовых свечах рассчитывается индикатор ADX(14), по которому рынок относится к одному из
 * режимов:
 *
 * <ul>
 *   <li>{@code TREND} — ADX ≥ {@value #ADX_TREND_THRESHOLD} (сильный направленный тренд)
 *   <li>{@code RANGE} — ADX ≤ {@value #ADX_RANGE_THRESHOLD} (боковик / флэт)
 *   <li>{@code NORMAL} — промежуточное состояние
 * </ul>
 *
 * <p>В зависимости от режима корректируется баланс, передаваемый в {@link UnifiedStrategy} для
 * расчёта размера позиции:
 *
 * <ul>
 *   <li>{@code TREND} → balance × 1.5 (более агрессивный сайзинг)
 *   <li>{@code RANGE} → balance × 0.5 (более консервативный сайзинг)
 *   <li>{@code NORMAL} → без изменений
 * </ul>
 *
 * <h2>2. Базовое решение</h2>
 *
 * <p>Основная логика входа/выхода делегируется {@link UnifiedStrategy#decide}. ML-слой применяется
 * <b>только к решениям {@code OPEN}</b>; сигналы {@code HOLD}/{@code CLOSE} пропускаются без
 * изменений.
 *
 ...
 * confirmation, а также временные признаки (час, день недели).
 *
 * <p>Модель {@code trade_classifier} возвращает вероятность выигрыша сделки. Порог фильтрации
 * адаптируется под режим:
 *
 * <ul>
 *   <li>{@code TREND} → базовый порог (по умолчанию {@value #DEFAULT_ML_MIN_PROBABILITY},
 *       переопределяется через системное свойство {@code ml.probability.threshold})
 *   <li>{@code NORMAL} → max(базовый, 0.48)
 *   <li>{@code RANGE} → max(базовый, 0.52)
 * </ul>
 *
 * <p>Сделки с вероятностью ниже порога отклоняются с действием {@code HOLD} и причиной {@code
 * ML_FILTERED_PROB_<значение>}.
 *
 * <h2>4. ML-сайзинг позиции</h2>
 *
 * <p>Если сделка прошла фильтр, итоговый объём корректируется коэффициентом от модели с
 * дополнительными поправками по режиму и уровню уверенности:
 *
 * <ul>
 *   <li>{@code RANGE} → ×0.75
 *   <li>{@code TREND} и prob ≥ 0.54 → ×1.15
 *   <li>prob ≥ 0.56 → дополнительно ×1.15
 *   <li>prob ≥ 0.60 → дополнительно ×1.10
 * </ul>
 *
 * <p>На основе нового {@code quantity} пересобирается {@link TradingDecision} и {@link Position}.
 *
 * <h2>5. Автодообучение модели</h2>
 *
 * <p>Вне режима бэктеста при каждом вызове {@link #decide} выполняется попытка переобучения модели
 * через {@link MlAutoTrainingService#tryRetrain}, использующий накопленные сделки из {@code
 * ml_strategy/data_pipeline/trades.csv}.
 *
 * <h2>6. Логирование и статистика</h2>
 *
 * <p>В {@link #onDailyReset()} выводится распределение режимов (Trend/Range/Normal), количество
 * отфильтрованных ML сделок и сводная статистика вероятностей: min/avg/max и распределение по
 * уровням (≥0.10, 0.20, 0.30, 0.40, 0.50).
 *
 * <h2>7. Обогащение причины сделки</h2>
 *
 * <p>К полю {@code reason} итогового решения добавляется контекст вида {@code
 * <REGIME>_ADX<значение>[_ML<вероятность>]_<исходная_причина>}, что упрощает последующий разбор
 * логов и пост-анализ результатов.
 *
 * @see UnifiedStrategy
...
public class RegimeAwareStrategyMl extends BaseStrategy {
  ...
  public RegimeAwareStrategyMl(
      UnifiedTraderConfig unifiedTraderConfig,
      TCSService tcsService,
      Config config,
      boolean isBacktest,
      boolean useMlFiltering,
      boolean useMlSizing) {
    ...
    this.mlAutoTrainingService =
        new MlAutoTrainingService(
            "ml_strategy/data_pipeline/trades.csv",
    ...
    logWithBacktest(
        "RegimeAwareStrategy ML v5: ML_Filter="
            + useMlFiltering
            + ", ML_Sizing="
            + useMlSizing
  ...
  private void loadPerTickerModels(UnifiedTraderConfig config) {
    ...
    logWithBacktest(
        "Loaded "
            + loadedCount
            + " per-ticker models out of "
            + config.getStocks().size()
            + " tickers");
  }
  ...
  public TradingDecision decide(
    ...
    // Create ML features
    TradeFeatures features = createTradeFeatures(ticker, hourCandles, decision, regime, adx);
    ...
    if (useMlFiltering && winProbability < regimeThreshold) {
      ...
      return new TradingDecision(
          "HOLD", "ML_FILTERED_PROB_" + String.format("%.2f", winProbability));
    }
  ...
  private TradeFeatures createTradeFeatures(
      String ticker,
      List<Candle> candles,
      TradingDecision decision,
      MarketRegime regime,
    ...
    double stopLoss =
        decision.updatedPosition.stopLoss != null
            ? decision.updatedPosition.stopLoss
            : entryPrice * 0.98;
    double takeProfit =
        decision.updatedPosition.takeProfit != null
            ? decision.updatedPosition.takeProfit
            : entryPrice * 1.06;
    ...
    return new TradeFeatures(
        ...
        entryTime.getHour(),
        entryTime.getDayOfWeek().getValue(),
        ticker,
        getStrategyName(),
        entryTime,
        entryPrice);
  ...
  private TradingDecision wrapDecision(
      TradingDecision decision, MarketRegime regime, double adx, double mlProb) {
  ...
  private double calculateAdx(List<Candle> candles, int period) {
    ...
    double adx =
        (diPlus + diMinus) > 0 ? Math.abs(diPlus - diMinus) / (diPlus + diMinus) * 100 : 0.0;
  ...
  protected void onDailyReset() {
    ...
    if (mlScoredTrades > 0) {
      ...
      String probabilitySummary =
          "RegimeAwareStrategy ML Probabilities: threshold="
              + String.format("%.2f", mlService.getProbabilityThreshold())
              + ", scored="
              + mlScoredTrades
              + ", min="
              + String.format("%.4f", minProbability)
              + ", avg="
              + String.format("%.4f", avgProbability)
              + ", max="
              + String.format("%.4f", maxProbability)
              + ", >=0.10="
              + above10
              + ", >=0.20="
              + above20
