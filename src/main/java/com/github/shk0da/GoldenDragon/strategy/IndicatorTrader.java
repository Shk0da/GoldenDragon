...
public class IndicatorTrader {
  ...
  public void run() {
    ...
    while (true) {
      stocks.forEach(
          stock -> {
            boolean hasNotActiveDetectAnomaly =
                null == anomalies.get(stock.getFigi())
                    || anomalies.get(stock.getFigi()).isBefore(now().minusMinutes(5));
            ...
            try {
              if (purchases.contains(stock.getFigi())) {
                List<HistoricCandle> m15candles =
  ...
  private boolean hasGlassAnomaly(GetOrderBookResponse orderBook) {
    ...
    // Покупка
    Double maxAsk =
        max(orderBook.getAsksList().stream().map(it -> toDouble(it.getPrice())).collect(toList()));
    // Продажа
    Double maxBid =
        max(orderBook.getBidsList().stream().map(it -> toDouble(it.getPrice())).collect(toList()));
  ...
  public static boolean calculateSignalDown(List<HistoricCandle> candles) {
    ...
    boolean maTrendDown =
        maWhite.get(maWhite.size() - 1).getClose() < maWhite.get(maWhite.size() - 1).getValue();
    ...
    boolean rsiCrossoverSma =
        rsi.get(rsi.size() - 1).getValue() < rsiSma.get(rsiSma.size() - 1).getValue();
    ...
    boolean obvCrossoverSma =
        obv.get(obv.size() - 1).getValue() < obvSma.get(obvSma.size() - 1).getValue();
  ...
  public static boolean calculateSignalUp(List<HistoricCandle> candles, double adxLevel) {
    ...
    boolean maSuperTrendUp =
        maWhite.get(maWhite.size() - 3).getValue() < maWhite.get(maWhite.size() - 2).getValue()
            && maWhite.get(maWhite.size() - 2).getValue()
                < maWhite.get(maWhite.size() - 1).getValue();
    boolean maWhiteUpperBlack =
        maWhite.get(maWhite.size() - 1).getValue() > maBlack.get(maBlack.size() - 1).getValue();
    boolean closeCrossoverMa =
        maWhite.get(maWhite.size() - 1).getClose() > maWhite.get(maWhite.size() - 1).getValue();
    ...
    boolean macdSuperTrendUp =
        macdSign.get(macdSign.size() - 3).getValue() < macdSign.get(macdSign.size() - 2).getValue()
            && macdSign.get(macdSign.size() - 2).getValue()
                < macdSign.get(macdSign.size() - 1).getValue();
    boolean macdUpperZero = macd.get(macd.size() - 1).getValue() > 0.000;
    boolean macdCrossoverSignal =
        macd.get(macd.size() - 1).getValue() >= macdSign.get(macdSign.size() - 1).getValue();
