...
public class LevelTrader {
    ...
    public void run() {
        ...
        ExecutorService executor =
                Executors.newFixedThreadPool(levelTraderConfig.getStocks().size());
        ...
        if (!isWorkingHours()) {
            ...
            var profitInPercents =
                    (tcsService.getTotalPortfolioCost() - initPortfolioCost)
                            / initPortfolioCost
                            * 100;
            var statsMessage =
                    "Day profit: "
            ...
            if (!orders.isEmpty()) {
                ...
                for (OrderInfo order : orders) {
                    var tickers =
                            tickerByOrderInfo.getOrDefault(order.tickerName, new ArrayList<>());
                    tickers.add(order);
    ...
    private void handleTicker(String name) {
        ...
        try {
            ...
            var currentPositionBalance =
                    null == currentPosition ? 0.0 : currentPosition.getBalance();
            if (0 != decision) {
            ...
            if (0 != currentPositionBalance
                ...
                if (currentPositionBalance > 0
                    ...
                    var currentPrice =
                            tcsService.getAvailablePrice(name, type, count, "bids", false);
                    expectedYield = (currentPrice - positionPrice) / positionPrice * 100;
                    var expectedYieldMessage =
                            name + ": " + String.format("%,.2f%s", expectedYield, "%");
                ...
                if (currentPositionBalance < 0
                        && (expectedYield > tpPercent || expectedYield < ((-1) * slPercent))) {
                    var currentPrice =
                            tcsService.getAvailablePrice(name, type, count, "asks", false);
    ...
    private int decision(TickerDayInfo tickerDayInfo) {
        ...
        var candles =
                getTickerCandles(
                        tickerDayInfo.getName(),
                        (INDICATORS_SHIFT + 1008),
        ...
        if (hasTrendUp) {
            var hasUpATR =
                    (startOfDay.getLow() + (atr - (atr * 0.2))) > (lastCandle.getClose() + tp);
        ...
        if (!hasTrendUp) {
            var hasDownATR =
                    (lastCandle.getClose() - tp) + (atr - (atr * 0.2)) < startOfDay.getHigh();
        ...
        try {
            ...
            var periodCandles =
                    tcsService.getCandles(
                            ticker, currentTime.minusMinutes(size * 5L), currentTime, period);
