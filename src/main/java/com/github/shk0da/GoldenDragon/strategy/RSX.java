...
public class RSX extends Rebalancing {
    ...
    public void run() throws Exception {
        ...
        Map<TickerInfo.Key, PortfolioPosition> targetPositions =
                topSymbols.stream()
                                ...
                                it -> {
                                    var pos =
                                            new PortfolioPosition(
                                                    it,
                                                    TickerType.STOCK,
                                                    100.0 / rsxConfig.getStockPortfolioMaxSize());
                        ...
                        .collect(
                                Collectors.toMap(
                                        it -> new TickerInfo.Key(it.getName(), it.getType()),
                                        it -> it));
    ...
    public List<String> topSymbols(List<TickerScan> tickers) {
        ...
        for (TickerScan ticker : tickers) {
            ...
            if (!tickerRepository.containsKey(
                    new TickerInfo.Key(ticker.getName(), TickerType.STOCK))) continue;
        ...
        List<Map<String, Object>> stocks =
                dataForFiler.stream()
                        .sorted(
                                (it1, it2) ->
                                        ((Double) it2.get("recommend1M"))
                                                .compareTo((Double) it1.get("recommend1M")))
                        .limit(100)
        ...
        stocks.forEach(
                stock -> {
                    List<HistoricCandle> candles =
                            tcsService.getLastCandles(
                                    (String) stock.get("symbol"), TickerType.STOCK, 128);
                    if (!candles.isEmpty() && candles.size() >= 2) {
                        var topCandles =
                                candles.stream().skip(candles.size() - 2).collect(toList());
                        double lastClose =
                                tcsService.convertQuotationToDouble(
                                        candles.get(candles.size() - 1).getClose());
                        double firstClose =
                                tcsService.convertQuotationToDouble(candles.get(0).getClose());
                        double topLastClose =
                                tcsService.convertQuotationToDouble(
        ...
        return stocks.stream()
                .sorted(
                        (it1, it2) ->
                                ((Double) it2.get("momentum"))
                                        .compareTo((Double) it1.get("momentum")))
