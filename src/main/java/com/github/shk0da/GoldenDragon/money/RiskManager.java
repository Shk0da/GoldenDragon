...
/**
 * Risk manager for trading limits and daily loss control.
...
public class RiskManager {
     ...
     */
    public RiskManager(double riskPerTradePercent, double maxDailyLossPercent, int maxConsecutiveLosses) {
    ...
    public void registerTrade(double pnl) {
        ...
        if (pnl >= 0) {
            ...
            newStats = new DailyStats(
                    oldStats.wins + 1,
                    oldStats.losses,
                    oldStats.dailyPnL + pnl,
                    oldStats.trades + 1
            );
        } else {
