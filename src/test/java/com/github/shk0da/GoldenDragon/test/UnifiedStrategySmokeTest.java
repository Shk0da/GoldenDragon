...
/**
 * Manual smoke test for UnifiedStrategy execution flow.
...
public class UnifiedStrategySmokeTest {
    ...
    public static void main(String[] args) {
        try {
            ...
            StrategyTestAdapter strategy = new StrategyTestAdapter(traderConfig, tcsService, config);
            ...
            try {
                TradingDecision buyDecision = new TradingDecision(
                        "OPEN",
                        "SMOKE_BUY",
                        1.0,
                        quantity,
                        ...
                        takeProfitPrice,
                        marketPrice,
                        new Position("BUY", marketPrice, stopLossPrice, takeProfitPrice, quantity, 0)
                );
            ...
            try {
                ...
                strategy.close(
                        ...
                        tickerInfo,
                        buyPosition,
                        new TradingDecision("CLOSE", "SMOKE_CLOSE_BUY", 0.0, buyPosition.quantity, null, null, marketPrice, new Position(config.cooldownCandles))
                );
                Thread.sleep(2000);
            ...
            try {
                ...
                strategy.close(
                        ...
                        tickerInfo,
                        sellPosition,
                        new TradingDecision("CLOSE", "SMOKE_CLOSE_SELL", 0.0, sellPosition.quantity, null, null, marketPrice, new Position(config.cooldownCandles))
                );
                Thread.sleep(2000);
    ...
    private static void verifyPositionExists(TCSService tcsService, String title, boolean expectedExists) {
        PositionInfo position = tcsService.getCurrentPositions(tickerType, testTicker);
        printPosition(tcsService, title);

        boolean actualExists = position != null && position.getBalance() != 0;
        if (expectedExists != actualExists) {
            out.println("Position verification FAILED: expectedExists=" + expectedExists + ", actualExists=" + actualExists);
        } else {
    ...
    private static final class StrategyTestAdapter extends UnifiedStrategy {
        ...
        private StrategyTestAdapter(UnifiedTraderConfig unifiedTraderConfig, TCSService tcsService, Config config) {
            super(unifiedTraderConfig, tcsService, config, false);
        }

        private void open(String name, TickerInfo ticker, List<Candle> candles, TradingDecision decision) {
            openPosition(name, ticker, candles, decision);
        }

        private void close(String name, TickerInfo ticker, Position position, TradingDecision decision) {
