...
public final class TCSServiceSmokeTest {
    ...
    private static void testPositions(final TCSService tcsService) {
        ...
        try {
            ...
            positions.forEach(
                    (key, pos) ->
                            out.println(
                                    "  " + pos.getTicker() + ": " + pos.getBalance() + " lots"));
        } catch (final Exception e) {
    ...
    private static void testOrderBook(final TCSService tcsService) {
        ...
        try {
            ...
            final Map<String, Map<Double, Integer>> prices =
                    tcsService.getCurrentPrices(key, false);

            out.println("Ticker: " + TEST_TICKER);
            out.println(
                    "Bids: " + prices.get("bids").size() + ", Asks: " + prices.get("asks").size());
    ...
    private static void testQuotation() {
        ...
        try {
            ...
            out.println(
                    "Quotation units: " + quotation.getUnits() + ", nano: " + quotation.getNano());
        ...
        try {
            ...
            final boolean buyResult =
                    tcsService.buyByMarket(
                            TEST_TICKER, TICKER_TYPE, TEST_CASH_AMOUNT, takeProfit, stopLoss);
    ...
    private static void testLimitBuy(final TCSService tcsService) {
        ...
        try {
            ...
            final boolean limitBuyResult =
                    tcsService
                            .buyLimit(TEST_TICKER, TICKER_TYPE, TEST_CASH_AMOUNT, limitPrice)
                            .isSuccess();
    ...
    private static void testLimitSell(final TCSService tcsService) {
        ...
        try {
            ...
            final double limitPrice =
                    bestBid > 0.0 ? Math.max(priceStep, bestBid - priceStep) : 0.0;
            ...
            final boolean limitSellResult =
                    tcsService
                            .sellLimit(TEST_TICKER, TICKER_TYPE, TEST_CASH_AMOUNT, limitPrice)
                            .isSuccess();
