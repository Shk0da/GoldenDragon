...
Python script for analyzing order book strategy performance from diagnostic logs.

**When asked to analyze logs, diagnostics, or metrics — always use this script instead of reading raw log files.**
...
# Custom paths
python scripts/analyze_strategy.py --csv path/to/metrics.csv --log path/to/diagnostics.log
```
...
| Section | Description |
|---------|-------------|
| Overall Performance | Net/gross P&L, win rate, profit factor, expectancy |
| P&L by Instrument | Per-ticker breakdown: trades, wins, fees |
| P&L by Signal | Performance per signal type (tradeFlow, microprice) |
| P&L by Direction | LONG vs SHORT comparison |
| Exit Reason Analysis | stop_loss, time_stop, rescreen_exit stats |
| Position Sizing | Units, entry price, notional, margin per trade |
| Hold Time Distribution | Histogram of position hold durations |
| Quality Score Analysis | Quality scores for wins vs losses |
| Fee Impact | Fee-to-gross ratio, per-instrument fee drag |
| Risk Metrics | Max drawdown, consecutive losses, largest win/loss |
| Entry Skip Analysis | Skip reason distribution from diagnostics log |
| Trade Flow Distribution | Histogram of flow values at skip events |
| Key Observations | Auto-detected issues and recommendations |

**Input files:**

- `orderbook-metrics.csv` — generated by `OrderBookMetricsCsvWriter` when `metricsCsvEnabled=true`
- `orderbook-diagnostics-replay.log` — generated by `OrderBookDiagnosticsReplayWriter` when `diagnosticsReplayEnabled=true`

**Enable diagnostics in `application.properties`:**
```properties
orderbook.diagnosticsEnabled=true
orderbook.diagnosticsReplayEnabled=true
orderbook.metricsCsvEnabled=true
```
